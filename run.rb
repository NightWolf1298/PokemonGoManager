...
POKE_GENIE_EDIT_LOC = Loc.new(10, 1010)
POKE_GENIE_EDIT_DISMISS_LOC = Loc.new(90, 175)
...
SCREENSHOT_POKEGENIE_RETRY = SSLoc.new(Loc.new(10, 1010), '#f1f3f5ff') # white background of Poke Genie IVision "unable to detect" error
SCREENSHOT_RENAME = SSLoc.new(Loc.new(1150, 1620), '#43d0a5ff') # greenish background of OK button

# Screenshot return values
RET_SCREENSHOT_OK = 0
RET_SCREENSHOT_FALLBACK = 1

# Miscellaneous settings
SCREENSHOT_FALLBACK_RETRIES = 5 # number of times we can retry for the "unable to detect" Poke Genie error
...
def adb_verify_screenshot(what, ssloc, fallback = nil, top = false)
  unless ADB_SCREENSHOT_VERIFY_ENABLE
    return RET_SCREENSHOT_OK
  end
  ...
  for attempt in 1..ADB_SCREENSHOT_VERIFY_RETRY_ATTEMPTS do
    ...
    if (fallback != nil)
      exp_fallback = ChunkyPNG::Color.from_hex(fallback.color)
      if pixel == exp_fallback
        return RET_SCREENSHOT_FALLBACK
      end
    end

    delay = attempt <= 2 ? ADB_SCREENSHOT_VERIFY_RETRY_DELAY / 2 : ADB_SCREENSHOT_VERIFY_RETRY_DELAY
    puts ">> Screenshot validation failed - '#{what}' was #{hex} but expected #{ssloc.color}, waiting #{delay} sec, attempt #{attempt} of #{ADB_SCREENSHOT_VERIFY_RETRY_ATTEMPTS}"
    sleep(delay)
  end
...
def adb_tap(what, loc, wait, top = false)
  loc = scale(loc, top)
  puts("adb_tap '#{what}' #{loc.x} #{loc.y}")
  if ADB_SEND_ENABLE
...
def adb_swipe(what, start, finish, duration, wait, top = false)
  start = scale(start, top)
  finish = scale(finish, top)
  puts "adb_swipe '#{what}' #{start.x} #{start.y} #{finish.x} #{finish.y} #{duration}"
  if ADB_SEND_ENABLE
...
def adb_backspace_swipe(what, loc, dx, hold_duration, swipe_duration, wait, top = false)
  loc = scale(loc, top)
  dx = scale(Loc.new(dx, 0), top).x # scale just the X coordinate
  puts "adb_backspace_swipe '#{what}' #{loc.x} #{loc.y}"
  if ADB_SEND_ENABLE
...
def adb_key(what, keycode, wait)
  puts "adb_key '#{what}' #{keycode}"
  if ADB_SEND_ENABLE
...
def adb_key_repeat(what, keycode, repeat, wait)
  puts "adb_key_repeat '#{what}' #{keycode} x#{repeat}"
...
row_even = true
adb_verify_screenshot('Pokemon Info Screen', SCREENSHOT_POKEMON_INFO) # make sure we start out in Pokemon info
for i in 1..POKEMON_TO_PROCESS do
  ...
  adb_tap('Pokemon Info Menu', POKEMON_INFO_MENU_LOC, DELAY_NO_VERIFY)
  adb_tap('Appraisal', POKEMON_INFO_APPRAISE_LOC, DELAY_NO_VERIFY)
  adb_tap('Appraisal Next', POKEMON_INFO_APPRAISE_DISMISS_LOC, DELAY_IVISION)

  # Wait for Poke Genie IVision to display
  for attempt in 1..SCREENSHOT_FALLBACK_RETRIES do
    ret = adb_verify_screenshot('Poke Genie IVision Results', SCREENSHOT_POKEGENIE, SCREENSHOT_POKEGENIE_RETRY)
    if ret == RET_SCREENSHOT_OK
      # worked this time
      break
    end
    if ret == RET_SCREENSHOT_FALLBACK
      # PokeGenie says "retry" so dismiss the screen and wait again
      puts ">> Poke Genie appraisal failed, retrying - attempt #{attempt} of #{SCREENSHOT_FALLBACK_RETRIES}"
      adb_tap('Poke Genie Edit', POKE_GENIE_EDIT_LOC, DELAY_NO_VERIFY)
      adb_tap('Poke Genie Edit Dismiss', POKE_GENIE_EDIT_DISMISS_LOC, DELAY_NO_VERIFY)
    end
  end

  # Dismiss the appraisal screen
  adb_tap('Dismiss Appraisal', POKEMON_INFO_APPRAISE_DISMISS_LOC, DELAY_NO_VERIFY)

  # Open rename dialog
  adb_tap('Rename', POKEMON_INFO_RENAME_LOC, DELAY_NO_VERIFY, true) # this button is offset on some phones due to the dead zone at the top
  ...
  adb_backspace_swipe('Remove Old Name', GBOARD_BACKSPACE, -200, DELETE_HOLD_DURATION, DELETE_SWIPE_DURATION, DELAY_NONE)

  # Paste new name
  adb_key('Paste New Name', KEYCODE_PASTE, DELAY_NONE)

  # Dismiss keyboard
  adb_tap('Dismiss Keyboard', POKEMON_INFO_RENAME_DISMISS_LOC, DELAY_NO_VERIFY)

  # Rename Pokemon
  adb_tap('Rename OK', POKEMON_INFO_RENAME_OK_LOC, DELAY_VERIFY)
  adb_verify_screenshot('Pokemon Info Screen', SCREENSHOT_POKEMON_INFO)

  # Swipe to next
  adb_swipe('Swipe To Next', POKEMON_INFO_SWIPE_START, POKEMON_INFO_SWIPE_END, POKEMON_INFO_SWIPE_DURATION, DELAY_NEXT_POKEMON)
